...
<head>
    ...
    <script>
        if(localStorage.getItem('logado') !== 'sim') {
            // Ajuste o caminho do login conforme sua estrutura, ex: ../../../login.html
    ...
    <style>
        /* 1. Override do Tema Global para Laranja (Alerta/Gatilho) */
        :root {
            --primary-color: #e17055; /* Laranja Queimado */
            --success-color: #00b894; /* Verde para Ação */
            --trigger-bg: #fff5f2;    /* Fundo suave para área de gatilho */
            --action-bg: #f0fdf4;     /* Fundo suave para área de ação */
        }

        /* 2. Estilos Exclusivos dos Cards de Gatilho (Não estão no style.css) */
        .trigger-list {
        ...
        .trigger-card {
            ...
            display: grid; 
            grid-template-columns: 50px 1fr 1fr; /* Icon | Contexto | Ação */
            overflow: hidden;
            animation: slideUp 0.4s ease;
        }

        /* Coluna do Ícone Lateral */
        .card-icon-col {
            background: #f1f2f6; 
        ...
        .card-label { 
            ...
            margin-bottom: 8px; display: block; opacity: 0.7; color: var(--text-muted);
        }
        ...
        .btn-card-delete {
            ...
            background: none; border: none; color: #fab1a0; 
            cursor: pointer; transition: 0.2s;
        }
        .btn-card-delete:hover { color: var(--danger-color); transform: scale(1.1); }

        /* Ajustes de Responsividade para o Grid do Card */
        @media (max-width: 768px) {
            .trigger-card { grid-template-columns: 1fr; }
            .card-icon-col { display: none; } /* Esconde ícone no mobile para ganhar espaço */
            .col-trigger { border-right: none; border-bottom: 1px dashed var(--border-color); }
        }
        ...
        @media print {
            .trigger-list, .card-panel, .app-header button { display: none !important; }
            #print-header { display: block; font-size: 1.5rem; margin-bottom: 20px; font-weight: bold; }
            #print-table { display: table; width: 100%; border-collapse: collapse; }
...
<body>
    ...
    <header class="app-header" data-aos="fade-down">
        <button class="btn-print-header" onclick="preparePrint()">
            <i class="fa-solid fa-print"></i> <span>Imprimir</span>
        </button>
        
        <h1>Rastreador de Gatilhos</h1>
        <div class="subtitle">Mapeando situações e respostas saudáveis</div>
    </header>
    ...
    <div id="print-header">
        Gatilhos do Esquema: <span id="printSchemaName"></span>
    </div>
    ...
    <script>
        ...
        function addCard() {
            ...
            if (!situation || !trigger) {
                alert("Preencha pelo menos a Situação e o Gatilho.");
                return;
            }
        ...
        function preparePrint() {
            ...
            cards.forEach(card => {
                const situation = card.querySelector('.col-trigger .card-text:first-of-type').innerText;
                const trigger = card.querySelector('.col-trigger .text-trigger.card-text').innerText;
                const action = card.querySelector('.col-action .card-text').innerText;

                const tr = document.createElement('tr');
                tr.innerHTML = `<td>${situation}</td><td>${trigger}</td><td>${action}</td>`;
